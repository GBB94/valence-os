...
**Account command center Release 2 — Operate implemented:** account Overview is now the lens-based command center. Operate combines material changes since explicit review and browser-local visit, deterministic attention, confirmed upcoming events, and the latest operator point of view. Ten rebuildable activity adapters preserve source truth and temporal semantics; append-only account/program checkpoints share review state safely with account-scoped Copilot runs. Prepare and Leadership are useful thin consumers over the same response, with their complete workflows remaining in Slices 2.2–2.3. Authority: `ACCOUNT-COMMAND-CENTER-SPEC.md`; decisions: D-117–D-119.
