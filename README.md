...
backend/
  ...
  migrations/               0001…0038 landed; Stages 10–14 are complete
...
**Account command center Release 2 — all three lenses implemented:** account Overview is now the lens-based command center. Operate combines material changes since explicit review and browser-local visit, deterministic attention, confirmed upcoming events, and the latest operator point of view. Prepare adds scoped meeting selection, fail-closed attendee resolution, attendee-related context, open threads, named evidence gaps, and explicit no-write pre-call/Copilot actions. Leadership adds independent governed statuses, evidence-aware forecast posture, scoped movement/exposure, explicit asks and escalations, contractual dates, and review provenance without inventing a composite health score. Ten rebuildable activity adapters preserve source truth and temporal semantics; append-only account/program checkpoints share review state safely with account-scoped Copilot runs. Slice 2.4 remains the activity-consumer and evidence review. Authority: `ACCOUNT-COMMAND-CENTER-SPEC.md`; decisions: D-117–D-121.
...
**Phase 3 feature-complete build — complete through Stage 14.** Production-mode items remain gated on the open §12 decisions and hosting approval. See `HANDOFF.md` for the current handoff.
