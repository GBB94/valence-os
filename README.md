...
**Account command center Release 2 — Operate and Prepare implemented:** account Overview is now the lens-based command center. Operate combines material changes since explicit review and browser-local visit, deterministic attention, confirmed upcoming events, and the latest operator point of view. Prepare adds scoped meeting selection, fail-closed attendee resolution, attendee-related context, open threads, named evidence gaps, and explicit no-write pre-call/Copilot actions. Ten rebuildable activity adapters preserve source truth and temporal semantics; append-only account/program checkpoints share review state safely with account-scoped Copilot runs. Leadership remains the next complete workflow in Slice 2.3. Authority: `ACCOUNT-COMMAND-CENTER-SPEC.md`; decisions: D-117–D-120.
