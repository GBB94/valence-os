...
- **Run the backend tests:** `cd backend && .venv/bin/python -m pytest`  (1,025 tests as of the Stage 18 hardening pass; `pyproject.toml` already sets `-q`, so do not pass it again)
- **Run the frontend tests:** `cd frontend && npm test`  (336 tests as of the Stage 18 hardening pass)
...
VISIBILITY-SPEC.md          Built additive visibility authority — six slices, all in force
SURFACE-USAGE-SPEC.md       Built Stage 17 authority — usage evidence and reversible retirement
CALL-COACHING-SPEC.md       Built Stage 18 authority — private deterministic text-first Coach
...
backend/
  ...
  migrations/               0001…0057 landed; current through Stage 18
...
**Visibility and surface usage — implemented:** `VISIBILITY-SPEC.md` makes absence, withholding, evidence age, plan variance, and explicit advocacy visible without inventing scores. `SURFACE-USAGE-SPEC.md` adds local content-free reach/engagement evidence, event-driven coverage, reversible presentation changes, retirement safety, and a redundancy checklist that deliberately does not pretend telemetry can judge product overlap. Authority: Stage 17; decisions D-251–D-342.

**Private Call Coach — implemented locally:** Coach supports standalone or explicitly account-linked completed-call review, exact-cited deterministic observations, one active focus, and single-skill text rehearsal. Analysis writes only the private coaching domain; Interaction logging and account-fact proposal drafting are explicit post-analysis commands with separate receipts and idempotent retries. Real-model, audio, live, sharing, manager, and longitudinal modes remain closed or evidence-gated. Authority: `CALL-COACHING-SPEC.md`; decisions D-343–D-351.
...
| **Visibility** | evidence decay/withholding, independent absence counters, explicit evaluators, plan variance, and dated advocacy tags without composite scores (migration 0054) |
| **17 · Surface usage** | local content-free reach/engagement evidence, monthly rollups, reversible retirement notes, event-driven coverage, screen weight, and the redundancy checklist (migrations 0055–0056) |
| **18 · Private Call Coach** | standalone or account-linked exact-cited review, one active focus, single-skill text rehearsal, private source controls, and explicit idempotent Interaction/Proposal Review bridges (migration 0057) |

**Feature-complete through the built local Stage 18 boundary and the full Account Path.** Suites: **1,025 backend, 336 frontend**, clean production build. Production-mode items remain gated on the open §12 decisions and hosting approval. See `HANDOFF.md` for the current handoff.

**Rendered verification still owed:** both-theme capture for the **Stage 16** drop zone (Slices 1–3)
and a click-through of the Stage 18 hardening routes. Stage 17 and VISIBILITY Slices 2–6 carry Zach's
explicit screenshot waiver. The Stage 16 capture block is recorded in
`design-screenshots/stage-16/VERIFICATION.md`; the Stage 18 validation session had no in-app browser
attached, so it claims automated tests/build only.
