...
| `call_coaching_endpoint` | Private call review and targeted rehearsal | Deterministic local contract; no real adapter | `backend/app/fixtures/coaching/*.json` | No runtime switch; real mode has no implementation | Separate transcript/reflection payload review; speaker-consent review; provider/model and region; retention/training terms; prompt-injection golden set; redacted logs; rollback configuration; logged decision |
