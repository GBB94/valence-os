...
_CONNECTIONS = (
    ...
    {
        "id": "product_telemetry_sink",
        "label": "Product measurement sink",
        "current_mode": "local_sqlite_only",
        "config_switch": "Disabled from Operations; VALENCE_OS_TELEMETRY_STRICT controls whether an invalid event is rejected or dropped",
        "fixture_glob": None,
        "real_requires": "Approved analytics vendor, processing region, retention and deletion terms, event allow-list review, pseudonymous identifier policy, and an explicit decision that behavioural data may leave the installation",
        "code_owner": "app/telemetry.py + app/routers/telemetry.py",
    },
    {
        "id": "document_drop_intake",
        "label": "Account drop zone (dropped and pasted documents)",
        "current_mode": "local_text_only_bytes_never_persisted",
        "config_switch": "No runtime switch; POST /api/accounts/{id}/intake/drops accepts text or text-file bytes",
        "fixture_glob": None,
        "real_requires": "Approved retention and deletion terms for customer document text, plus a reviewed answer before any binary format (PDF/DOCX/OCR/audio) or any file storage is added",
        "code_owner": "app/intake_drop.py + app/intake_kind.py + app/routers/intake_drops.py",
    },
    {
        "id": "hosting",
