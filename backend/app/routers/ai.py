...
from .. import audit, execution_ops, extraction_runs, extractor, repo, stage7, stage9
...
# Run + proposal persistence lives in `app.extraction_runs.persist_run` — the one proposal store's
# one writer, shared with the drop zone, `.eml` ingestion, Call Coach, and the seed. The router
# only validates HTTP input and calls down.
...
def run_extraction(b: ExtractionRequest, conn: sqlite3.Connection = Depends(get_conn)):
    ...
    run_id = extraction_runs.persist_run(conn, account_id=b.account_id, program_id=b.program_id,
