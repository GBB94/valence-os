...
**Next step is operator review, not another numbered build stage.** Run the Stage 8 end-to-end demo and inspect the Stage 9 portfolio/playbook surfaces with mock data. Do not connect a real external source.
...
**Running Phase 3 test count: 229** (was 67 at Phase 2 close). Backend still requires Python 3.12 (`.venv/bin/python -m pytest`).
...
# Tests (from backend/) — 229 tests, all green
...
Backend routers are in `backend/app/routers/`, frontend views in `frontend/src/views/`. The table below is the pre-Phase-3 foundation (migrations 0001–0010); Phase 3 additions through Stage 8 are migrations 0011–0024 and are summarized above.
...
- **No numbered Phase 3 stage remains.** Operator review and the still-gated production decisions are next.
