...
- **Stage 5.5 — done.** Whitespace map, value realization ledger, funding intelligence (migrations 0017-0019, `app/expansion.py`, `app/routers/expansion.py`). The counting rule (§1.1) is enforced in schema and service: segments are the only additive dimension and the API rejects headcounts exceeding the partition's FTE; composite views return `additive: false`; row paid seats is the max across use cases, never the sum. Cell state is **derived from four stored facts, never written** — there is no PATCH path to them, only `/set-fact` with a required reason writing `cell_state_history`. Cohort privacy floor (default 25, per-account setting) suppresses sub-floor values and refuses to build sub-floor composite views. `metric_observations` gained `population_segment_id`, which is what makes realization computable. ARR is derived once and stored; revenue movement reports absolutes with its base, never a blended rate. Frontend: three new Commercial sub-tabs (`Whitespace.jsx`, `ValueLedger.jsx`, `Funding.jsx`), whitespace as the default. Screenshots in `design-screenshots/stage-5.5/` (both themes). D-84.
- **Remaining, re-sequenced by `EXPANSION-ENGINE-SPEC.md` §0 (D-83):** **Stage 6** generators (Part 5), now able to consume Stage 5.5's cells, value targets, and funding pools directly → **Stage 7** triggers+calendar+change-detection (6.1, 6.2, 3.9 — incl. the "no validated second champion" play, the expansion/pull-signal plays, and the `dedupe_key` fix) → **Stage 7.5** qualification + pre-agreed triggers + renewal center + growth plan → **Stage 8** CONNECTIONS.md + e2e demo → **Stage 9** portfolio analytics + playbook library.

**The build order changed — read `EXPANSION-ENGINE-SPEC.md` §0 before starting Stage 6.** That spec (v3, 2026-07-31, D-81/D-83) is the whitespace / value-ledger / funding module, and it is **in force and interleaved**, not a later phase. It inserts **Stage 5.5** — whitespace map, value ledger, funding intelligence — *before* Stage 6, because Stage 6's business case builder assembles "scorecard vs. the agreed bar, funding waterfall, named expansion lines," and every one of those nouns is defined in the expansion spec. Building Stage 6 first means building it against data that does not exist. §3's signals engine is absorbed into **Stage 7** (where §6.1 already lists the expansion-signal trigger), and new **Stage 7.5** and **Stage 9** follow.

Stage 7 must also fix `play_runs.dedupe_key`, currently `play_id:object_id` under a global UNIQUE index — a play can never fire twice for the same object, which makes any recurring signal structurally impossible. It is a one-line index change now and a migration against accumulated play history later.

Its §0 table lists four pre-existing gaps; two are fixed (below), two are open and belong in Stage 5.5: `portfolio_io._INSERT_ORDER` still stops at migration 0005 (so MAP promotion, people layers, cadence, ingestion, and relationship intelligence are silently dropped from "full" account export), and search / source-citation lookup / the Operations screen lag the current schema.
...
**Running Phase 3 test count: 172** (was 67 at Phase 2 close). Backend still requires Python 3.12 (`.venv/bin/python -m pytest`).
